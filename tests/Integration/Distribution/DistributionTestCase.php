...
abstract class DistributionTestCase extends TestCase
{
    ...
    {
        $geoColumns = ['guid', 'stateid', 'lgaid', 'wardid', 'dpid', 'geo_level', 'geo_value', 'title', 'geo_string'];
        $stateColumns = ['StateId', 'Fullname'];
        $lgaColumns = ['LgaId', 'Fullname', 'StateId'];
        $wardColumns = ['wardid', 'ward', 'lgaid'];
        $dpColumns = ['dpid', 'dp', 'wardid'];
        $tokenColumns = ['tokenid', 'uuid'];
        $mobColumns = ['hhid', 'dp_id', 'etoken_id', 'etoken_serial', 'family_size', 'allocated_net', 'collected_date'];
        $distColumns = ['dp_id', 'hhid', 'etoken_id', 'etoken_serial', 'collected_nets', 'collected_date'];
        ...
        if (!$this->tableHasColumns('sys_geo_codex', $geoColumns)
            ...
            $this->markTestSkipped('Distribution schema not available');
        }
    }
}
