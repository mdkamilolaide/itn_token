<?php
/*
 *  DataTable API entry point.
 *  Loads bootstrap + JWT + scope, then dispatches the request to
 *  lib/api/table/{qid}.php. Each handler file inherits this script's
 *  local scope ($token, $v_g_*, etc.) and echoes a DataTables-shaped
 *  JSON payload: { draw, recordsTotal, recordsFiltered, data }.
 */
 ...
 *  Dispatch
 *
 *  $qid is resolved through lib/api/table/_routes.php (qid => relative path).
 ...
 */
$qid = CleanData('qid');
$routes = require __DIR__ . '/lib/api/table/_routes.php';

if ($qid !== '' && isset($routes[$qid]) && preg_match('#^[A-Za-z0-9_\-/]+\.php$#', $routes[$qid])) {
    $handler = __DIR__ . '/lib/api/table/' . $routes[$qid];
    if (is_file($handler)) {
        require $handler;
        return;
    }
