(function onLoadScript() {
  ...
  document
    .querySelector(".main-menu, .navbar-header")
...
async function runMap() {
  ...
  var start_date =
    mob_date.split(" to ")[0] == undefined ? "" : mob_date.split(" to ")[0];
  var end_date =
    mob_date.split(" to ")[1] == undefined ? "" : mob_date.split(" to ")[1];
  ...
  try {
    ...
    alert.Error(
      "ERROR",
      "Check your Internet, kindly Refresh your browser " + error,
    );
  }
  ...
  // Add Marker
  var markers =
    mob_result && Array.isArray(mob_result.mob_data) ? mob_result.mob_data : [];
  ...
  const addMarker = (coords) => {
    ...
    var infoWindow = new google.maps.InfoWindow({
      content:
        ...
        "</b><br>Family size: <b>" +
        coords.family_size +
        "</b><br>Allocated Net: <b>" +
        coords.allocated_net +
        "</b><br>e-Token Serial: <b>" +
        coords.etoken_serial +
        "</b><br>HH Mobilizer: <b>" +
        coords.mobilizer +
        "</b><br>Date: <b>" +
        coords.collected_date +
        "</b> ",
    });
...
async function loadLGA() {
  await axios
    .post(
      common.DataService + "?qid=gen003",
      JSON.stringify(localStorage.getItem("state_id")),
    )
    .then((response) => {
      ...
      for (let i = 0; i < select.length; i++) {
        ...
        for (index in allLga) {
          select[i].options[select[i].options.length] = new Option(
            allLga[index].lga,
            allLga[index].lgaid,
          );
        }
    ...
    .then((response) => {
      ...
      for (let i = 0; i < select.length; i++) {
        ...
        for (index in mobilizerData) {
          select[i].options[select[i].options.length] = new Option(
            mobilizerData[index].first +
              " " +
              mobilizerData[index].last +
              "  (" +
              mobilizerData[index].loginid +
              ")",
            mobilizerData[index].loginid,
          );
        }
...
async function loadDpPerWard(wardid) {
  ...
  overlay.show();
  await axios
    .get(url + "?qid=gen006&wardid=" + wardid)
    .then((response) => {
