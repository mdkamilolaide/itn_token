...
((root) => {
  ...
  utils.checkIfEmpty = (v) =>
    v === null || v === "" || v === undefined ? "Nil" : v;
  ...
  utils.validatePaste = (event) => {
    var pasteData = (event.clipboardData || window.clipboardData).getData(
      "text",
    );
    if (!/^\d+$/.test(pasteData)) {
      event.preventDefault();
  ...
  utils.dataQuery = (qid, params) => {
    ...
    return axios.get(url, {
      params: Object.assign({ qid: qid }, params || {}),
    });
  };
  ...
  utils.progressBarWidth = (issue, used) =>
    utils.percentageUsed(issue, used) + "%";
  ...
  utils.useEventBus = (handlers) => {
    ...
    if (!Vue || !Vue.onMounted || !Vue.onBeforeUnmount) {
      console.warn(
        "[utils.useEventBus] Vue 3 lifecycle hooks not available. Did you load vue.global.js?",
      );
      return;
    }
    var pairs = [];
    if (typeof handlers.onReset === "function")
      pairs.push(["g-event-reset-form", handlers.onReset]);
    if (typeof handlers.onRefresh === "function")
      pairs.push(["g-event-refresh-page", handlers.onRefresh]);
    if (typeof handlers.onGoto === "function")
      pairs.push(["g-event-goto-page", handlers.onGoto]);
    if (typeof handlers.onUpdate === "function")
      pairs.push(["g-event-update", handlers.onUpdate]);
    if (typeof handlers.onUpdateUser === "function")
      pairs.push(["g-event-update-user", handlers.onUpdateUser]);

    Vue.onMounted(() => {
      pairs.forEach((p) => {
        utils.bus.on(p[0], p[1]);
      });
    });
    Vue.onBeforeUnmount(() => {
      pairs.forEach((p) => {
        utils.bus.off(p[0], p[1]);
      });
    });
  };
  ...
  const cloneDeepPreserveFns = (value, seen) => {
    ...
    if (Array.isArray(value)) {
      ...
      for (var i = 0; i < value.length; i++)
        out.push(cloneDeepPreserveFns(value[i], seen));
      return out;
    }
      ...
      const init = () => {
        ...
        try {
          ...
          if (p && typeof p.catch === "function") {
            p.catch((e) => {
              console.error("[utils.ApexChart] async render failed:", e);
            });
          }
        } catch (e) {
      ...
      watch(
        ...
        (n) => {
          if (chart && typeof chart.updateSeries === "function") {
            try {
              chart.updateSeries(cloneDeepPreserveFns(n || []));
              return;
            } catch (e) {
  ...
  (() => {
    ...
    const bind = () => {
      ...
      if (
        Ctor &&
        Ctor.prototype &&
        Ctor.prototype._hideModal &&
        !Ctor.prototype._hideModal._itnPatched
      ) {
      ...
      if (
        Ctor &&
        Ctor.prototype &&
        Ctor.prototype._showElement &&
        !Ctor.prototype._showElement._itnPatched
      ) {
      ...
      if (!$.fn.modal._itnPatched) {
        ...
        for (var k in originalModal)
          if (originalModal.hasOwnProperty(k)) $.fn.modal[k] = originalModal[k];
