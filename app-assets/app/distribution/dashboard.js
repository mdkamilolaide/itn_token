...
const PageBody = {
  setup() {
    const refreshAllData = () => {
      bus.emit("g-event-refresh-page", { page: "", distributionPage: "" });
    };
...
const DistributionGeneralStats = {
  setup() {
    ...
    const fetchData = (qid, onSuccess) => {
      return axios
        .get(common.DataService + "?qid=" + qid)
        .then((response) => {
          var data =
...
const DistributionLgaAggregateTable = {
  setup() {
    ...
    const refreshData = () => {
      overlay.show();
      var qid = "";
      if (appState.aggregate.title === "LGA" || appState.aggregate.title === "")
        qid = "404a";
    ...
    onMounted(() => {
      ...
      try {
        var containers = document.querySelectorAll(
          ".distribution-lga-perfect-scroll-grid",
        );
...
const DailyAggregateTable = {
  setup() {
    ...
    const goToTopSummaryPage = (data) => {
      ...
      appState.dailyAggregate.chartTitle =
        fmtUtils.displayDate(data && data.date) + " Summary";
      refreshData();
    };
    ...
    const handleRowClick = (g) => {
      ...
      var title = appState.dailyAggregate.title;
      if (title === "Date")
        goToTopSummaryPage({ date: g.title, page: "top_summary" });
      else if (title === "LGA")
        goToLGASummaryPage({
          lgaId: g.id,
          lgaName: g.title,
    ...
    const series = computed(() =>
      chartData.value && chartData.value[0] ? chartData.value[0] : [],
    );
    ...
    onMounted(() => {
      ...
      try {
        var containers = document.querySelectorAll(
          ".daily-aggregate-perfect-scroll-grid",
        );
        containers.forEach((el) => {
          psInstances.push(new PerfectScrollbar(el));
        });
