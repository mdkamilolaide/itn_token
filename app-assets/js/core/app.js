...
(function (window, document, $) {
  ...
  //Check if Pass has been first Reset
  if ($("#v_g_pass_change").val() == "Yes") {
    $("#change-password .modal-actions").remove();
...
if (form.length) {
  form.each(function () {
    if ($(".new_password").length > 0) {
      $.validator.addMethod("checklower", function () {
        let value = $(".new_password").val();
        return /[a-z]/.test(value);
      });
      $.validator.addMethod("checkupper", function () {
        let value = $(".new_password").val();
        return /[A-Z]/.test(value);
      });
      $.validator.addMethod("checkdigit", function () {
        let value = $(".new_password").val();
        return /[0-9]/.test(value);
      });
      $.validator.addMethod("pwcheck", function () {
        let value = $(".new_password").val();
        return (
      ...
      $.validator.addMethod("notEqualTo", function () {
        var notEqual = true;
    ...
    $this.validate({
      ...
      messages: {
        new_password: {
          ...
          checklower: "Need at least 1 Small Letter",
          checkupper: "Need at least 1 Capital Letter",
          checkdigit: "Need at least 1 Number",
          notEqualTo: "Enter a different Password from the Old One",
        },
        confirm_password: {
...
$("#change-password").on("submit", function (e) {
  ...
  userDetails.push({
    ...
    loginid: $("#v_g_loginid").val(),
  });
  var self = this;
  var url = common.DataService;
  overlay.show();
  axios
    .post(url + "?qid=011", JSON.stringify(userDetails))
    .then(function (response) {
      if (response.data.result_code == "200") {
        location.href = $("#v_g_prefix").val() + "logout?state=pass";
        overlay.hide();
      } else {
        overlay.hide();
        alert.Error("Error", "Wrong Old Password");
      }
    })
