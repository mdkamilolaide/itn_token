...
class INineC{
    ...
    public function BulkSave($bulk_ob){
        #data structure
        /*
        *   [uid,lgaid,wardid,userid,latitude,longitude,aa,ab,ac,ad,ae,af,ag,ah,domain,app_version,capture_date]
        */
        ...
        foreach($bulk_ob as $a){
            $query = "INSERT INTO `mo_form_i9c` (`uid`,`wardid`,`lgaid`,`userid`,`latitude`,`longitude`,`aa`,`ab`,`ac`,`ad`,`ae`,`af`,`ag`,`ah`,`domain`,`app_version`,`capture_date`,`created`) VALUES 
            (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $this->db->executeTransaction($query,array($a['uid'],$a['wardid'],$a['lgaid'],$a['userid'],$a['latitude'],$a['longitude'],$a['aa'],$a['ab'],$a['ac'],$a['ad'],$a['ae'],$a['af'],$a['ag'],$a['ah'],$a['domain'],$a['app_version'],$a['capture_date'],$date));
        ...
        if(strlen($error_message)>0){
            ...
            $error_to_write = "\r\nForm i9c error, message: $error_message\r\nData:".json_encode($bulk_ob)."\r\n$date\r\n";
