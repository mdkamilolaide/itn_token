...
CREATE TABLE `smc_drug_administration` (
  ...
  `resode_issue_id` int(11) NOT NULL,
  ...
  KEY `resode_issue_id` (`resode_issue_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `smc_icc_collection` (
  ...
  `calculated_used` int(11) DEFAULT NULL COMMENT 'Keeps records of drug administration used',
  `calculated_partial` int(11) DEFAULT NULL,
...
) ENGINE=InnoDB AUTO_INCREMENT=6893 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `usr_user_activity` (
  ...
  KEY `userid` (`userid`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=1305 DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
DROP TRIGGER IF EXISTS `trg_add_to_calculated_used`;
CREATE DEFINER=`root`@`localhost` TRIGGER trg_add_to_calculated_used
AFTER INSERT ON smc_drug_administration
FOR EACH ROW
BEGIN
  UPDATE smc_icc_collection
  SET calculated_used = COALESCE(calculated_used, 0) + NEW.drug_qty,
  calculated_partial = COALESCE(calculated_partial, 0) + NEW.redose_count
