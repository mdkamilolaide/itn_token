...
CREATE TABLE `smc_icc_collection` (
  ...
  `downloaded` tinyint(4) DEFAULT NULL COMMENT 'Downloaded quantity',
  `download_date` datetime DEFAULT NULL COMMENT 'Downloaded date',
  `confirm` tinyint(4) DEFAULT NULL COMMENT '0 - rejected, 1 - confirmed, null not acted upon',
  `confirm_date` datetime DEFAULT NULL COMMENT 'confirmation date',
  `return_full` tinyint(4) DEFAULT NULL COMMENT 'Total Qty of full commodity returned',
  `return_patial` tinyint(4) DEFAULT NULL COMMENT 'Total Qty of partial commodity returned',
  `return_date` datetime DEFAULT NULL COMMENT 'returned date',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `smc_icc_download_log` (
  ...
  `created` datetime DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  KEY `aggr_id` (`aggr_id`) USING BTREE,
  KEY `cdd_lead_id` (`cdd_lead_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `smc_icc_issue` (
  ...
  `issue_drug` varchar(100) DEFAULT NULL,
...
CREATE TABLE `smc_icc_reconcile` (
  ...
  `cdd_lead_id` int(11) NOT NULL,
  `drug` varchar(100) NOT NULL,
  `remaining` int(11) NOT NULL DEFAULT 0 COMMENT 'Total used',
  `full` int(11) DEFAULT NULL COMMENT 'total full reconcile',
  `partial` int(11) DEFAULT NULL COMMENT 'total partial reconciled',
  `wasted` int(11) DEFAULT NULL COMMENT 'total adjustment',
  `loss` int(11) NOT NULL DEFAULT 0 COMMENT 'discrepancy if any, 0 by default',
  `loss_reason` varchar(255) DEFAULT NULL COMMENT 'reason for discrepancy ',
  `receiver_id` int(11) DEFAULT NULL COMMENT 'HFW id',
  `is_reconcile_ready` tinyint(4) NOT NULL DEFAULT 0,
  `returned` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'indicator to show if it has been reconciled',
  `device_serial` varchar(50) DEFAULT NULL,
  `app_version` varchar(50) DEFAULT NULL,
...
) ENGINE=InnoDB AUTO_INCREMENT=6701 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `usr_user_activity` (
  ...
  KEY `userid` (`userid`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=1293 DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
DROP TRIGGER IF EXISTS `update_aggregator_after_admin_insert`;
CREATE DEFINER=`root`@`localhost` TRIGGER `update_aggregator_after_admin_insert` AFTER INSERT ON `smc_drug_administration` FOR EACH ROW BEGIN
    -- Deduct the sum of full and redose from a_aggregator.qty
    UPDATE smc_icc_aggragator
    SET qty = qty - (NEW.drug_qty + NEW.redose_count)
    WHERE cdd_lead_id = NEW.user_id AND drug = NEW.drug;
END;

DROP TRIGGER IF EXISTS `after_admin_track_receive`;
CREATE DEFINER=`root`@`localhost` TRIGGER `after_admin_track_receive` AFTER INSERT ON `smc_drug_administration` FOR EACH ROW BEGIN
    -- Track update a_receive.full and a_receive.partial 
    UPDATE smc_icc_reconcile
    SET `full` = `full` + NEW.drug_qty,
        partial = `partial` + NEW.redose_count
    WHERE smc_icc_reconcile.recon_id = (SELECT smc_icc_reconcile.recon_id FROM smc_icc_issue INNER JOIN smc_icc_reconcile ON smc_icc_issue.issue_id = smc_icc_reconcile.issue_id WHERE smc_icc_reconcile.cdd_lead_id = NEW.user_id AND smc_icc_reconcile.drug = NEW.drug AND smc_icc_issue.drug_qty >= (smc_icc_reconcile.`full`+smc_icc_reconcile.partial+(NEW.drug_qty+NEW.redose_count)) LIMIT 1);
END;

DROP TRIGGER IF EXISTS `after_issue_insert`;
CREATE DEFINER=`root`@`localhost` TRIGGER `after_issue_insert` AFTER INSERT ON `smc_icc_issue` FOR EACH ROW BEGIN
    DECLARE agg_count INT;
    DECLARE receive_count INT;

    -- Check if a row with the same cdd_id and drug exists in the a_aggregator table
    SELECT COUNT(*) INTO agg_count 
    FROM smc_icc_aggragator 
    WHERE cdd_lead_id = NEW.cdd_lead_id AND drug = NEW.issue_drug;

    IF agg_count > 0 THEN
        -- If it exists, update the qty and total
        UPDATE smc_icc_aggragator
        SET qty = qty + NEW.drug_qty, total_qty = total_qty + NEW.drug_qty
        WHERE cdd_lead_id = NEW.cdd_lead_id AND drug = NEW.issue_drug;
    ELSE
        -- If it does not exist, insert a new row
        INSERT INTO smc_icc_aggragator (cdd_lead_id, drug, qty, total_qty)
        VALUES (NEW.cdd_lead_id, NEW.issue_drug, NEW.drug_qty, NEW.drug_qty);
    END IF;

    -- Check if the issue_id already exists in the a_receive table
    SELECT COUNT(*) INTO receive_count 
    FROM smc_icc_reconcile 
    WHERE issue_id = NEW.issue_id;

    IF receive_count = 0 THEN
        -- If it does not exist, insert a new row
        INSERT INTO smc_icc_reconcile (issue_id, cdd_lead_id, drug, `full`, `partial`, wasted, returned)
        VALUES (NEW.issue_id, NEW.cdd_lead_id, NEW.issue_drug, 0, 0, 0, 0); -- Assuming 'returned' is 0 by default
    END IF;
END;